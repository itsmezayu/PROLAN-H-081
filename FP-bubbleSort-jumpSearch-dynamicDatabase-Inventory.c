...
int main() {
    ...
    do {
    	...
    	case 3:
		    ...
		    // Pastikan data sudah diurutkan berdasarkan kriteria yang dipilih
		    bubbleSort(item, jumlahItem, kriteria, 1);
		
		    if (kriteria == 3) {
		        printf("\n Masukkan quantity yang ingin dicari: ");
		        scanf("%d", &searchQTY);
		        int hasil = jumpSearchQTY(item, jumlahItem, searchQTY);
		        if (hasil != -1) {
		            printf("\n Item ditemukan:\n");
		            printData(&item[hasil], 1);
		        } else {
		        ...
		        scanf(" %[^\n]", searchQuery);
		        int found = 0; // Flag untuk menandai apakah ada item yang ditemukan
		        printf("\n Item yang ditemukan:\n");
		        for (int i = 0; i < jumlahItem; i++) {
		            if ((kriteria == 1 && strcmp(item[i].name, searchQuery) == 0) ||
		                (kriteria == 2 && strcmp(item[i].category, searchQuery) == 0)) {
		                printData(&item[i], 1);
		                found = 1; // Set flag ke 1 jika ada item yang ditemukan
		            }
		        }
		        if (!found) {
