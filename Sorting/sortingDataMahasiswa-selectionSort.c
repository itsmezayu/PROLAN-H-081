...
void selectionSort(Mahasiswa mhs[], int n, int kriteria, int ascending) {
    for (int i = 0; i < n - 1; i++) {
        int minOrMaxIdx = i;
        ...
        for (int j = i + 1; j < n; j++) {
            int condition;
            if (kriteria == 1) { // NPM
                condition = strcmp(mhs[j].npm, mhs[minOrMaxIdx].npm) < 0;
            } else if (kriteria == 2) { // Nama
                condition = strcmp(mhs[j].nama, mhs[minOrMaxIdx].nama) < 0;
            } else if (kriteria == 3) { // Prodi
                condition = strcmp(mhs[j].prodi, mhs[minOrMaxIdx].prodi) < 0;
            } else { // IPK
                condition = mhs[j].ipk < mhs[minOrMaxIdx].ipk;
            }
        ...
        mhs[i] = mhs[minOrMaxIdx];
        mhs[minOrMaxIdx] = temp;
    }
}
...
int main() {
    ...
    selectionSort(mhs, jumlahMahasiswa, kriteria, ascending == 1);
