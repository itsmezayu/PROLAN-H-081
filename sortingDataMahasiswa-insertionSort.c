...
void insertionSort(Mahasiswa mhs[], int n, int kriteria, int ascending) {
    for (int i = 1; i < n; i++) {
        Mahasiswa key = mhs[i];
        int j = i - 1;
        ...
        while (j >= 0) {
            ...
            if (kriteria == 1) { // NPM
                condition = strcmp(mhs[j].npm, key.npm) > 0;
            } else if (kriteria == 2) { // Nama
                condition = strcmp(mhs[j].nama, key.nama) > 0;
            } else if (kriteria == 3) { // Prodi
                condition = strcmp(mhs[j].prodi, key.prodi) > 0;
            } else { // IPK
                condition = mhs[j].ipk > key.ipk;
            }
...
int main() {
    ...
    insertionSort(mhs, jumlahMahasiswa, kriteria, ascending == 1);
